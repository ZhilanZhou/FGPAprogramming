...
module screenmem #(
   ...
   input wire [$clog2(Nloc)-1 : 0] screen_addr,     // Address for specifying memory location
                                             //   num of bits in addr is ceiling(log2(number of locations))
   input wire [$clog2(Nloc)-1+2 : 0] mips_addr,
   ...
   always_ff @(posedge clock)                // Memory write: only when wr==1, and only at posedge clock
      if(wr)
         mem[mips_addr[$clog2(Nloc)-1+2 : 2]] <= din;

   assign to_mips_dout = {28'b0, mem[mips_addr[$clog2(Nloc)-1+2 : 2]]};
   assign dout = mem[screen_addr];                  // Memory read: read continuously, no clock involved
